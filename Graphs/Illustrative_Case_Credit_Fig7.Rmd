...
categorical_atts <- c("CheckingStatus", "CreditHistory", "Purpose", "Savings",
                      "PresentEmploymentSince", "SexMarritalStatus", 
                      "Guarantors", "Property", "OtherInstallment", "Housing",
                      "Job", "Telephone", "ForeignWorker", "CustomerClass")
...
# Only Keep numerical features in the data
...
base_RU_map <- base_total %>% 
  ...
  geom_text(mapping = aes(label = noise), hjust = 0.1, vjust =1 ) +
...
for (i in seq_along(d)) {
  ...
  each_frontier_total[i, "risk"] <- model_metric(formula = risk_model_formula,
                                               data = dplyr::select(each_frontier_Data, 
                                                             -all_of(pred_var)),
                                               model = model,
                                               metric = metric)
  ...
  if (SV_calc) {# Compute exact Shapley values
    ...
    SV_Risk <- Shapley.Value(formula = risk_model_formula,
                             data = dplyr::select(each_frontier_Data, -all_of(pred_var)),
                             model = model, 
                             metric = metric)
      
    SV_Utility <- Shapley.Value(formula = utility_model_formula,
                                data = dplyr::select(each_frontier_Data, -all_of(conf_var)),
                                model = model, 
                                metric = metric)
  ...
  each_frontier_comb_SV <- full_join(x = SV_Risk, 
                                     y = SV_Utility, 
                                     by = "Variable") %>%
    dplyr::select(Variable, SV.x, SV.y) %>%
  ...
  each_frontier_comb_SV_d <- left_join(x = each_frontier_comb_SV, 
    ...
    dplyr::select(Variable, dominated, {{sel_method}})
...
#### Figure 7a: Comparing Benchmark with Risk-Only Masking Approaches
```{r plot_comparing_methods_risk_only, include = TRUE, fig.keep = 'all'}
...
graph_all <- All_Risk_Utility_Tradeoff %>% 
  filter((candidate_att %in% c("CreditAmount only","Updated Efficient Frontier"))) %>%#
  mutate(candidate_att = ifelse(candidate_att == "Updated Efficient Frontier", "Proposed (risk)", candidate_att)) %>%
  mutate(candidate_att = ifelse(candidate_att == "CreditAmount only", "Benchmark", candidate_att)) %>%
  group_by(candidate_att) %>%
  mutate(across(c("risk", "utility"),  \(x) lead(x, n = 1), .names = "{.col}_b")) %>%
  mutate(masked_att = ifelse(masked_att == "Duration_M", "Duration", masked_att)) %>%
  ungroup() %>%
...
```{r initializing_wc, include=FALSE}
...
for (i in seq_along(d)) {
  ...
  next_att <- each_frontier_comb_SV_d %>% 
    ...
    # If there is no numericla attribute on the frontier, chooose the topic numerical feature under the metric
  if(length(next_att)==0) {
  next_att <- each_frontier_comb_SV_d %>%
    mutate(Variable = str_remove(Variable, "_Masked")) %>%
    filter(Variable %in% c(numerical_atts)) %>%
...
#### Figure 7b: Comparing Benchmark and Weighted-Cost Masking Approaches
```{r plot_comparing_methods_wc, include = TRUE, fig.keep = 'all'}
# Expanding the base method results to look like that of two_att_total
base_total_ex <- base_total %>% 
  rename(noise_total = noise) %>%
  add_column(candidate_att = paste(conf_var, "only"),

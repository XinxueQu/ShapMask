ShapMask <- function(mask, model, metric, sim) {
  ...
  d_max <- ifelse(mask %in% adp_vec, 0.3 + 3*num_conf_vars, 0.15 + 1.5*num_conf_vars)
  
  # Change in noise delta
  delta <- d_max/(10*num_conf_vars + 1)
                            ...
                            for (d in d_vec) {
                              ...
                              if (sel != "benchmark" & sel != "random") {
                                ...
                                if (SV_calc) {# Compute exact Shapley values
                                  ...
                                  SV_Risk <- Shapley.Value.Est2(formula = risk_model_formula,
                                                               data = dplyr::select(Data_Masked, !{{ pred_var }}),
                                                               M = 2, Q = 10,
                                                               model = model, metric = metric)

                                  SV_Utility <- Shapley.Value.Est2(utility_model_formula,
                                                                  data = dplyr::select(Data_Masked, !{{ conf_var }}),
                                                                  M = 2, Q = 10,
                                                                  model = model, metric = metric)
